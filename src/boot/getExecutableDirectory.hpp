...
namespace Boot {
    ...
    [[nodiscard]] inline path getExecutableDirectory() noexcept {
            ...
            for(DWORD cap = 260; attempts < maxAttempts; cap *= 2, attempts++) {
                if (cap > 32768) Debug::exit(Debug::FILESYSTEM_CANNOT_GET_EXECUTABLE_PATH);
                exePathStr.resize(cap);
                const DWORD len = GetModuleFileNameW(NULL, exePathStr.data(), cap);
                if (len == 0) Debug::exit(Debug::FILESYSTEM_CANNOT_GET_EXECUTABLE_PATH);
                if (len < cap - 1) {
            ...
            if (exeDict.empty()) Debug::exit(Debug::FILESYSTEM_CANNOT_GET_EXECUTABLE_PATH);
            return exeDict;
            ...
            if (len == PATH_MAX) for(ssize_t cap = PATH_MAX * 2; cap <= PATH_MAX * 8; cap *= 2) {
                ...
                if (cap >= PATH_MAX * 16) Debug::exit(Debug::FILESYSTEM_CANNOT_GET_EXECUTABLE_PATH);
            }
            if (len <= 0) Debug::exit(Debug::FILESYSTEM_CANNOT_GET_EXECUTABLE_PATH);
            buffer[len] = '\0';
            ...
            for (; attempts < maxAttempts; attempts++) {
                ...
                if (rc == 0) break;
                if (cap > PATH_MAX * 8) Debug::exit(Debug::FILESYSTEM_CANNOT_GET_EXECUTABLE_PATH);
                buf.resize(cap);
            }
            if (attempts >= maxAttempts) Debug::exit(Debug::FILESYSTEM_CANNOT_GET_EXECUTABLE_PATH);
