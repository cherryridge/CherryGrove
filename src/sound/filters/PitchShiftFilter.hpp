...
namespace Sound {
    ...
    struct PitchShiftFilter;

    namespace detail {
        inline constexpr double PI = 3.141592653589793;
        inline constexpr float WINDOW_RANGE_MS = 20.0f, SEARCH_RANGE_MS = 10.0f, OVERLAP_FRAC = 0.5f;

        inline void resample(const vector<float>& in, vector<float>& out) noexcept {
    ...
    struct PitchShiftFilterInstance : public SoLoud::FilterInstance {
        ...
        PitchShiftFilterInstance(PitchShiftFilter* parent) noexcept : parent(parent) {}
        void filterChannel(float* buffer, u32 sampleCount, float sampleRate, double playTime, u32 currentChannel, u32 channelCount) noexcept;
    };
    ...
    struct PitchShiftFilter : public SoLoud::Filter {
        ...
        SoLoud::FilterInstance* createInstance() noexcept { return new PitchShiftFilterInstance(this); }
    };

    inline void PitchShiftFilterInstance::filterChannel(float* buffer, u32 sampleCount, float sampleRate, double playTime, u32 currentChannel, u32 channelCount) noexcept {
        if (!buffer || sampleCount == 0 || parent->pitch == 1.0f) return;
        vector<float> stretched = detail::wsola(buffer, sampleCount, sampleRate, 1.0f / pow(2.0f, parent->pitch)), finalBuf(sampleCount);
        detail::resample(stretched, finalBuf);
